...
DEFAULT_CLASS_IDS: List[int] = []  # TODO: Inspect the Coco dataset and add classes
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        default="/dev/video3",
        help="Camera device or index passed to OpenCV (default: /dev/video3)",
    )
    parser.add_argument(
        "--model",
        type=str,
        default=str(Path(__file__).resolve().parent / "yolov8s.pt"),
        help="Path to YOLOv8 model weights (default: local yolov8s.pt)",
    )
    parser.add_argument(
        "--conf",
        type=float,
...
def ensure_model(model_path: str) -> str:
    """
    Ensure the requested model exists locally. If not, fall back to the
    built-in YOLO weight name so Ultralytics downloads it automatically.
    """
    path = Path(model_path)
    if path.exists():
        return str(path)

    print(f"[INFO] Model '{model_path}' not found locally. Using '{path.name}' from Ultralytics hub.")
    return path.name
